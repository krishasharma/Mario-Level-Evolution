...
class Individual_Grid(object):
    ...
    def mutate(self, genome):
        ...
        # Simple mutation: randomly change a few tiles
        mutation_rate = 0.05  # 5% chance for each tile to mutate
        
        for y in range(height):
            for x in range(1, width - 1):  # Avoid first and last columns
                if random.random() < mutation_rate:
                    genome[y][x] = random.choice(options)
        
        # Ensure the ground is always solid
        genome[height-1] = ['X' for _ in range(width)]
        
        # Preserve Mario's start position and end-level flag
        genome[14][0] = 'm'
        genome[7][-1] = 'v'
    ...
    def generate_children(self, other):
        ...
        #parent’s genome up to the position and switch to the second parent’s genome afterwards
        left = 1
        right = width - 1
        for y in range(height):
            for x in range(left, right):
                ...
                if random.random() < 0.5:
                    new_genome[y][x] = self.genome[y][x] #uniform crossover
                else:
                    new_genome[y][x] = other.genome[y][x] #uniform crossover
...
def generate_successors(population):
    ...
    elitism_count = len(population) // 10
    results.extend(population[:elitism_count])
    ...
    # Generate children until we reach the original population size
    ...
    # Trim excess individuals if we've generated too many
